...
//       // Send the request
//       const response = await axios.post('http://localhost:3000/menu/add', {
//         items: newItems
//       });

//       // Handle the response
//       if (response.data && response.data.message === 'Menu items added successfully') {
//         // Update menu items with the new items
...
//             const newItem = {
//               ...item,
//               _id: response.data.items[index]?._id,
//               order: response.data.items[index]?.order
//             };
//             currentItems.push(newItem);
//           });

//           // Sort items by order
//           return currentItems.sort((a, b) => a.order - b.order);
//         });
...
//       alert(err.response?.data?.message || 'Failed to add menu items');
//     }
//   };
...
export default function AddMenuItems() {
  ...
  const handleAddToMenu = async (items, type) => {
    try {
      ...
      console.log(`Adding ${items.length} ${type} to menu`);
      ...
      const newItems = items.map((item, index) => {
        ...
        if (type === 'pages') {
          ...
          newItem.pageId = item._id; // Include reference to the original page
        } else if (type === 'categories') {
          newItem.slug = item.slug || '';
          newItem.categoryId = item._id; // Include reference to the original category
        } else if (type === 'custom') {
      ...
      console.log('Sending new items to server:', JSON.stringify(newItems, null, 2));
  
      // Send the request with more detailed content
      const response = await axios.post('http://localhost:3000/menu/add', {
        items: newItems,
        type: type  // Explicitly include the type in the request
      });
      ...
      // Handle the response
      if (response.data) {
        // Instead of manipulating the state directly, fetch fresh data
        const updatedMenuResponse = await axios.get('http://localhost:3000/menu/all');
        console.log('Updated menu items:', updatedMenuResponse.data);
        setMenuItems(updatedMenuResponse.data);
      ...
      alert(err.response?.data?.message || 'Failed to add menu items. Check console for details.');
    }
  };
  ...
  const handleAddCustomLink = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/menu/all');
      console.log('Updated menu after adding custom link:', response.data);
      setMenuItems(response.data);
      setActiveTab('custom');
      setCustomUrl('');
      setCustomLinkText('');
  ...
  return (
    <div className="flex justify-between p-4">
      ...
      <div className="w-2/3 bg-white p-4 shadow-sm">
        ...
        <div className="mt-10">
          {menuItems.length === 0 ? (
            <p className="text-gray-500 italic">No menu items added yet. Select items from the left panel to add to your menu.</p>
          ) : (
            <ul className="space-y-2 w-full">
              {/* Render all menu items in a single list */}
              {organizedItems.topLevel.map(item => 
