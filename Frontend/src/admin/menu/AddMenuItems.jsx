...
export default function AddMenuItems() {
  ...
  const handleAddToMenu = async (items, type) => {
    try {
      ...
      // Create new items with proper ordering and required fields
      const newItems = items.map((item, index) => ({
        title: type === 'categories' ? item.category_name : item.title,
        type: type,
        order: menuItems.length + index + 1, // Use existing menu length for order
        parent: null,
        status: 'active',
        ...(type === 'categories' && { 
          categoryId: item._id,
      ...
      console.log('Sending menu items to server:', newItems);
      ...
      if (response.data.message === 'Menu items added successfully') {
        // Update menu items with the new items
        setMenuItems(prevItems => {
          const currentItems = [...prevItems];
          // Add the new items with their server-assigned IDs
          const updatedItems = [
            ...currentItems,
